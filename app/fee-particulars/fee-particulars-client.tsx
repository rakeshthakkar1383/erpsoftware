...
const emptyForm: Record<string, string> = { class_name: "", particular_name: "", amount: "", duration_months: "12", fee_type_id: "", fee_category: "School", school_id: "", trust_id: "" }
              ...
              {filtered.map((p: any, i: number) => (
                <tr key={p.id}>
                  ...
                  <td className="flex items-center gap-1 px-3 py-2">
                    ...
                    <button className="ml-1 text-blue-600 hover:underline" onClick={() => { setEditing(p); setForm({ class_name: p.class_name || "", particular_name: p.particular_name || "", amount: p.amount || "", duration_months: String(p.duration_months || "12"), fee_type_id: p.fee_type_id ? String(p.fee_type_id) : "", fee_category: p.fee_category || "School", school_id: p.school_id ? String(p.school_id) : "", trust_id: p.trust_id ? String(p.trust_id) : "" }); setMessage(""); setModal(true) }}>Edit</button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-lg rounded-lg bg-white p-6 shadow-xl">
            ...
            <div className="grid gap-3">
              ...
              <select className="w-full rounded border p-3 text-sm" value={form.duration_months || "12"} onChange={set("duration_months")}>
