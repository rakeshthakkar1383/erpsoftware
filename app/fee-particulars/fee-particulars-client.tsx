...
export default function FeeParticularsClient({ initialParticulars }: { initialParticulars: any[] }) {
  ...
  const handleSave = async () => {
    ...
    if (editing) { await updateFeeParticular(editing.id, toFD(form)) } else { await addFeeParticular(toFD(form)) }
  ...
  const filtered = particulars.filter((p: any) => !q || [p.class_name, p.particular_name, String(p.amount)].some((v: any) => v?.toLowerCase().includes(q)))
  ...
  return (
    <div>
      ...
      <div className="mb-4">
        ...
        <span className="ml-2 text-sm text-slate-500">{filtered.length} particulars</span>
      </div>
      {filtered.length === 0 ? <p>No fee particulars found.</p> : (
        <div className="overflow-x-auto rounded border">
          <table className="min-w-full divide-y divide-slate-200 text-left text-sm">
            <thead className="bg-slate-50 uppercase text-slate-600">
              <tr><th className="px-3 py-2">#</th><th className="px-3 py-2">Class</th><th className="px-3 py-2">Particular Name</th><th className="px-3 py-2">Amount</th><th className="px-3 py-2">Actions</th></tr>
            </thead>
              ...
              {filtered.map((p: any, i: number) => (
                <tr key={p.id}>
                  ...
                  <td className="flex gap-2 px-3 py-2">
                    <button className="text-blue-600 hover:underline" onClick={() => { setEditing(p); setForm({ class_name: p.class_name || "", particular_name: p.particular_name || "", amount: p.amount || "" }); setMessage(""); setModal(true) }}>Edit</button>
